...
import com.gestaosaude.myapp.config.KafkaProperties;
...
class GestaosaudeKafkaResourceIT {
    ...
    void setup() {
        ...
        GestaosaudeKafkaResource kafkaResource = new GestaosaudeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gestaosaude-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/gestaosaude-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
